...
스킬은 Claude Code 관례대로 [`.claude/skills/`](.claude/skills/)에 들어 있습니다. 이 repo를 클론하거나, 각 스킬 폴더를 프로젝트(또는 `~`)의 `.claude/skills/` 아래로 복사하면 바로 인식됩니다. 각 스킬은 `SKILL.md`(정의) + 보조 자산(`assets/`·`references/`·`agents/`)으로 구성됩니다.
