...
- **소개 슬라이드 소스:** [`deck/`](deck/) — dc-runtime 번들 HTML(`index.html`) + 동일출처 `vendor/`(React/ReactDOM/Babel)

> 이 repo가 단일 통합 저장소입니다 — 스킬(6종) · 프로토타입(`prototype/`) · 소개 슬라이드(`deck/`). (이전 `relay-deck`·`relay-prototype` repo는 여기로 통합됨.)
